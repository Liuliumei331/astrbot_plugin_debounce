...
class DebouncePlugin(Star):
    ...
    async def _create_fake_event_for_telegram(self, original_event, message_text, message_components):
        """专门为 Telegram 平台创建伪造事件"""
        from astrbot.core.platform import AstrBotMessage, MessageType
        from astrbot.core.platform.sources.telegram.tg_event import TelegramPlatformEvent
        
        # 创建新的 AstrBotMessage（逐个属性设置）
        new_msg = AstrBotMessage()
        new_msg.message_str = message_text
        new_msg.message_id = f"fake_{original_event.message_obj.message_id}"
        new_msg.session_id = original_event.session_id
        new_msg.sender = original_event.message_obj.sender
        new_msg.message = message_components
        new_msg.type = original_event.message_obj.type
        new_msg.self_id = original_event.get_self_id()
        new_msg.group_id = original_event.get_group_id() or ""
        new_msg.raw_message = None
        
        # 找到 Telegram 适配器
        if hasattr(self, 'context') and self.context:
            platforms = self.context.platform_manager.get_insts()
            for adapter in platforms:
                if hasattr(adapter, 'client') and hasattr(original_event, 'client'):
                    if adapter.client == original_event.client:
                        # 标记这个消息需要跳过防抖（防止死循环）
                        self.skip_debounce_msg_ids.add(new_msg.message_id)
                        
                        # 创建 Telegram 事件
                        fake_event = TelegramPlatformEvent(
                            message_str=message_text,
                            message_obj=new_msg,
                            platform_meta=adapter.meta(),  # meta() 是方法，不是属性
                            session_id=original_event.session_id,
                            client=adapter.client,
                        )
                        fake_event.is_wake = True  # 标记为唤醒事件
                        adapter.commit_event(fake_event)
                        logger.debug(f"[Debounce] 已提交 Telegram 唤醒事件")
                        return True
        
        return False
    
    async def _send_fake_event(self, original_event: AstrMessageEvent, message_text: str):
        """伪造一个消息事件并发送到 EventBus"""
        try:
            from astrbot.core.message.components import Plain
            from astrbot.api.message_components import Plain as PlainComponent
            
            platform_name = original_event.get_platform_name()
            logger.debug(f"[Debounce] 尝试为平台 {platform_name} 伪造事件")
            
            # 保留原始消息的富文本组件 (图片、表情等)
            original_message = original_event.message_obj.message
            new_message_components = []
            
            # 遍历原始消息组件，保留非 Plain 类型的组件
            for component in original_message:
                if not isinstance(component, PlainComponent):
                    new_message_components.append(component)
            
            # 在开头添加合并后的文本
            new_message_components.insert(0, PlainComponent(message_text))
            
            # 根据平台类型处理
            if platform_name == "telegram":
                # Telegram 平台：创建伪造事件让 LLM 处理
                logger.debug(f"[Debounce] Telegram 平台，创建伪造事件")
                
                success = await self._create_fake_event_for_telegram(
                    original_event, message_text, new_message_components
                )
                
                if success:
                    logger.debug(f"[Debounce] Telegram 伪造事件成功")
                else:
                    logger.warning(f"[Debounce] Telegram 伪造事件失败：未找到适配器")
            
            else:
                # 其他平台使用原有逻辑（StarTools）
